...
const repos 	= {
	'source': require('pushover')(config.source, { autoCreate: config.arg != 'default' }),
	'content': require('pushover')(config.content, { autoCreate: config.arg != 'default' }),
	'folk': require('pushover')(config.folk, { autoCreate: config.arg != 'default' })
}
...
var auth_access = {
    ...
    var def = Q.defer();
	  ...
	  if (auth === void 0) {
	    ...
	    var sql = 'SELECT password FROM user WHERE username=:username';
	  	// console.log('auth', { username: creds[0] }, { password: creds[1] });
    ...
    creds = { username: creds[0], password: creds[1] };
		db.selectOne('user_access', { username: creds.username }).then(function(user){
			if(user) def.resolve(user); else def.reject();
		}).catch(function(ex){
    ...
    var def = Q.defer();
		db.selectOne('permission', { url: repo }).then(function(permission){
			if(permission) {
				if(permission.private == 'YES') {
					def.resolve({ accept: (user.user_id == permission.user_id), error: 'this private repository' });
				} else if(permission.anonymous == 'YES') {
					def.resolve({ accept:true, error:null });
				} else {
					if(user.level > 0 && user.user_id != permission.user_id) {
		    		return db.select('repository_contributor', { repository_id: permission.repository_id, user_id: user.user_id }).then(function(roles){
			  			def.resolve({ accept:roles.length > 0, error:'You have no permission to information' });
		    		});
		    	} else {
			  		def.resolve({ accept:true, error:'' });
		    	}
				}
			} else {
		  	def.resolve({ accept:false, error:'Not found repository directory.' });
			}
		}).catch(function(ex){
	  	def.resolve({ accept:false, error:ex });
		});
    return def.promise;
	}
}
...
var control_git = {
	...
	email: function(template_name, options, data, req){
  	var def = Q.defer();
		...
		changeset.render(data, function (err, result) {
		  ...
		  auth_access.username(req.headers).then(function(user){
		  	scopt.user = user;
		    return db.selectOne('permission', { url: req.repo });
		  }).then(function(r){
	  		scopt.notify = r.notify == 'YES' ? true : false;
	  		scopt.private = r.private == 'YES' ? true : false;
	  		scopt.repository_id = r.repository_id;

		  	if(scopt.notify && !scopt.private) {
	  			var sql = "SELECT u.email FROM repository_contributor c JOIN user u ON c.user_id = u.user_id " 
									+ "WHERE (c.permission = 'Administrators' OR c.permission = 'Subscribe') AND c.repository_id = :repository_id";
				...
				} else if(scopt.private) {
		  		return db.query("SELECT email FROM user_access WHERE level < 2", {}).then(function(u){
			  		var mail_to = mailOptions.to.join('|');
			  		u.forEach(function(user){ 
			  			if((mail_to).indexOf(user.email) < 0 && mailOptions.bcc.join('|').indexOf(user.email) < 0) {
			  				mailOptions.bcc.push(user.email);
			  			}
			  		});
			  	}).catch(function(ex){
		  		...
		  		mailOptions.bcc = [];
		  		return db.query("SELECT email FROM user_access WHERE level < 2", {}).then(function(u){
			  		u.forEach(function(user){ mailOptions.to.push(user.email); });
			  	}).catch(function(ex){
		  			def.reject({ error: ex.toString() });
				  });
				}
	...
	folk: function(url, dir_name){
		var self = this;
    var def = Q.defer();

		repos['folk'].mkdir('./', function(){
			self.cmd('git', [ 'clone', url, dir_name, '--bare' ], config.folk, true).then(function(){
				def.resolve(true);
			});
		});
    ...
    var def = Q.defer();
		...
		repos[row.config].exists(row.dir_name, function(exists){
			if(!exists) {
				if(row.config == 'folk') {
					// Make dir
					self.folk(row.url, row.dir_name).then(function(){
						def.resolve(true);
					}).catch(function(ex){
						def.reject(ex);
					});
				} else {
					repos[row.config].create(row.dir_name, function(err){
