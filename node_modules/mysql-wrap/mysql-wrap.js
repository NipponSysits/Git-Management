...
var createMySQLWrap = function (connection, options) {
    ...
    var prepareTypesRows = function(item){
        if(typeof item == 'string') {
            item = "'"+item+"'";
        } else if(typeof item == 'object') {
            if(item == null || item == undefined) {
                item = "null";
            } else if(isNaN(item) || isNaN(item.value)) {
                item = "0";
            } else {
                item = "'"+item.value+"'";
            }
    ...
    self.query = function (sqlOrObject, valuesOrCallback, callbackOrNothing) {
        var queryFormat = function(sql, vals) {
            if (!vals) {
                return sql;
            }
            return sql.replace(/\:(\w+)/g, function (txt, key) {
                if (vals.hasOwnProperty(key)) {
                    return prepareTypesRows(vals[key]);
                }
                return txt;
            }.bind(this));
        }
        ...
        statement.sql = queryFormat(statement.sql, valuesOrCallback);
    ...
    var prepareWhereEquals = function (whereEquals) {
        ...
        var sql = _.map(whereEquals, function (val, key) {
            values.push(key, val);
            return '`' + key + "` "+ (val == null ? "IS NOT NULL" : "= '" + val + "'");
        }, '').join(' AND ');
    ...
    self.select = function (tableOrObject, whereEquals, callback) {
        ...
        return self.query(
            'SELECT ' + selectedFieldsSQL(statement.fields) + ' FROM ' + '`' + statement.table + '`  ' + where.sql +
    ...
    self.selectOne = function (tableOrObject, whereEquals, callback) {
        ...
        return self.one(
            ...
            ' FROM `'+ statement.table + '`' + where.sql,
            values,
            callback
        );
    };
    ...
    var prepareInsertRows = function (rowOrRows) {
        ...
        var processValuesSQL = function (row) {
            return '(' + _.map(fields, function (field) {
                ...
                return prepareTypesRows(row[field]); // '?'
            }) + ')';
        };
    ...
    self.update = function (table, setData, whereEquals, callback) {
        var prepareSetRows = function (setData) {
            ...
            var sql = ' SET ' + _.map(setData, function (val, key) {
                values.push(key, val);
                return '`'+key+'` = '+prepareTypesRows(val);
            }).join(', ');
        ...
        return self.query('UPDATE `'+table+'` ' + set.sql + where.sql, values, callback);
    };

    self.delete = function (table, whereEquals, callback) {
        var where = prepareWhereEquals(whereEquals);
        var values = [table].concat(where.values);
        return self.query('DELETE FROM `'+table+'`' + where.sql, values, callback);
    };
